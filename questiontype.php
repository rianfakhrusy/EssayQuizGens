 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_essayquizgens extends question_type {
    ...
    public function get_question_options($question) {
        global $DB;
        $question->options = $DB->get_record('qtype_essayquizgens_options',
    ...
    public function save_question_options($formdata) {
        ...
        $options = $DB->get_record('qtype_essayquizgens_options', array('questionid' => $formdata->id));
        if (!$options) {
            $options = new stdClass();
            $options->questionid = $formdata->id;
            $options->id = $DB->insert_record('qtype_essayquizgens_options', $options);
        }
        ...
        $options->graderinfo = $this->import_or_save_files($formdata->graderinfo,
                $context, 'qtype_essayquizgens', 'graderinfo', $formdata->id);
        ...
        $options->chapter = $formdata->chapter;
        $options->time = $formdata->time;
        $options->difficulty = $formdata->difficulty;

        $DB->update_record('qtype_essayquizgens_options', $options);
    }

    protected function initialise_question_instance(question_definition $question, $questiondata) {
        parent::initialise_question_instance($question, $questiondata);

        $question->chapter = $questiondata->options->chapter;
        $question->time = 5;
    ...
    public function delete_question($questionid, $contextid) {
        ...
        $DB->delete_records('qtype_essayquizgens_options', array('questionid' => $questionid));
    ...
    public function response_formats() {
        return array(
            'editor' => get_string('formateditor', 'qtype_essayquizgens'),
            'editorfilepicker' => get_string('formateditorfilepicker', 'qtype_essayquizgens'),
            'plain' => get_string('formatplain', 'qtype_essayquizgens'),
            'monospaced' => get_string('formatmonospaced', 'qtype_essayquizgens'),
            'noinline' => get_string('formatnoinline', 'qtype_essayquizgens'),
        );
    }
    ...
    public function response_required_options() {
        return array(
            1 => get_string('responseisrequired', 'qtype_essayquizgens'),
            0 => get_string('responsenotrequired', 'qtype_essayquizgens'),
        );
    }
    ...
    public function response_sizes() {
        ...
        for ($lines = 5; $lines <= 40; $lines += 5) {
            $choices[$lines] = get_string('nlines', 'qtype_essayquizgens', $lines);
        }
        return $choices;
    }
    ...
    public function attachments_required_options() {
        return array(
            0 => get_string('attachmentsoptional', 'qtype_essayquizgens'),
            1 => '1',
            2 => '2',
            3 => '3'
        );
    ...
    public function move_files($questionid, $oldcontextid, $newcontextid) {
        ...
        $fs->move_area_files_to_new_context($oldcontextid,
                $newcontextid, 'qtype_essayquizgens', 'graderinfo', $questionid);
    }

    protected function delete_files($questionid, $contextid) {
        parent::delete_files($questionid, $contextid);
        $fs = get_file_storage();
        $fs->delete_area_files($contextid, 'qtype_essayquizgens', 'graderinfo', $questionid);
    }
}
